...
public class YahooApiManager {
    ...
    private WeatherData parseJson(JSONObject json) throws ApiException {
        ...
        try {
            ...
            int highTomorrow = forecasts.getJSONObject(1).getInt("high");
            int lowTomorrow = forecasts.getJSONObject(1).getInt("low");
            String descriptionTomorrow = forecasts.getJSONObject(1).getString("text");

            data = new WeatherData(city, currentTemperature, highTemperature, lowTemperature, description, highTomorrow, lowTomorrow, descriptionTomorrow);
